  ...
  const renderModal = () => {
    ...
    const modalMarkup = (
      ...
      >
        {/* Centered Modal Card */}
        <div 
          className="relative w-full max-w-2xl sm:max-w-3xl max-h-[90vh] sm:max-h-[85vh] bg-[#0D0D0D] border border-[rgba(228,222,210,0.16)] rounded-3xl shadow-2xl flex flex-col overflow-hidden text-[#F6F4F1] animate-in zoom-in-95 duration-200"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Subtle glow accents */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#F95C4B]/5 rounded-full blur-3xl pointer-events-none -mr-20 -mt-20" />
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#161616] rounded-full blur-3xl pointer-events-none -ml-20 -mb-20" />

          {/* Modal Header */}
          <div className="relative z-10 flex items-center justify-between px-5 sm:px-6 py-4 border-b border-[rgba(228,222,210,0.12)] bg-[#121212]/90">
            <div className="flex items-center space-x-3">
              <div className="w-9 h-9 rounded-xl bg-[#1C1C1C] border border-[rgba(249,92,75,0.3)] flex items-center justify-center text-[#F95C4B]">
                <Layers className="w-5 h-5" />
              </div>
              <div>
                <h3 id="industry-modal-title" className="text-base sm:text-lg font-bold text-[#F6F4F1] tracking-tight">
            ...
            <button
              type="button"
              onClick={handleClose}
              className="text-[#A8A196] hover:text-[#F6F4F1] hover:bg-[#1C1C1C] p-2 rounded-xl transition border border-transparent hover:border-[rgba(228,222,210,0.12)]"
          ...
          {/* Modal Search Bar */}
          <div className="relative z-10 px-5 sm:px-6 py-3.5 bg-[#090909] border-b border-[rgba(228,222,210,0.08)]">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-[#121212] border border-[rgba(228,222,210,0.14)] rounded-xl pl-10 pr-10 py-2.5 text-sm text-[#F6F4F1] placeholder-[#A8A196]/60 focus:outline-none focus:ring-2 focus:ring-[#F95C4B]/40 focus:border-[#F95C4B] transition"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery("")}
                  className="absolute right-3.5 top-3 text-[#A8A196] hover:text-[#F6F4F1] p-0.5"
                  title="Clear search"
                >
                  <X className="w-4 h-4" />
                </button>
          ...
          <div className="relative z-10 flex items-center justify-between border-b border-[rgba(228,222,210,0.1)] bg-[#121212] px-5 sm:px-6 py-2.5 text-xs">
            <div className="flex items-center space-x-2 sm:space-x-3">
              <button
                ...
                className={`flex items-center space-x-1.5 py-1.5 px-3 rounded-lg font-semibold transition ${
                  activeTab === "search"
                    ? "bg-[#1C1C1C] text-[#F95C4B] border border-[rgba(249,92,75,0.3)] shadow-sm"
                    : "text-[#A8A196] hover:text-[#F6F4F1] hover:bg-[#161616]"
                }`}
              >
                <Search className="w-3.5 h-3.5" />
                <span>Quick Search</span>
                {searchResults.length > 0 && searchQuery && (
                  <span className="text-[10px] bg-[#F95C4B]/20 text-[#F95C4B] px-1.5 py-0.2 rounded-full font-bold">
          ...
          {selectedIndustries.length > 0 && (
            <div className="relative z-10 px-5 sm:px-6 py-2 bg-[#141414] border-b border-[rgba(228,222,210,0.08)] flex flex-wrap items-center gap-1.5 max-h-24 overflow-y-auto">
              <span className="text-[11px] font-medium text-[#A8A196] mr-1">Selected:</span>
              {selectedIndustries.map((ind) => (
                <span
                  key={ind.id}
                  className="inline-flex items-center space-x-1.5 bg-[#1C1C1C] text-[#F6F4F1] border border-[rgba(249,92,75,0.4)] text-xs font-semibold px-2.5 py-0.5 rounded-lg transition"
                >
                  <span>{ind.name}</span>
                  <button
                    type="button"
          ...
          <div className="relative z-10 flex-1 overflow-hidden flex flex-col min-h-[260px] sm:min-h-[320px]">
            ...
            {activeTab === "search" && (
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 divide-y divide-[rgba(228,222,210,0.06)]">
                {searchResults.length > 0 ? (
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left p-3 rounded-xl flex items-start justify-between transition group border ${
                              isSelected
                                ? "bg-[#1C1515] text-[#F95C4B] border-[rgba(249,92,75,0.4)] shadow-sm"
                                : "bg-[#101010] hover:bg-[#161616] text-[#F6F4F1] border-[rgba(228,222,210,0.06)] hover:border-[rgba(228,222,210,0.14)]"
                            }`}
                          >
                            <div className="flex flex-col pr-2 min-w-0">
                              <span className="text-xs font-semibold group-hover:text-[#F95C4B] transition truncate">
                                {ind.name}
                              </span>
                            ...
                            <div className="flex items-center space-x-2 shrink-0 mt-0.5">
                              {isSelected ? (
                                <div className="w-5 h-5 rounded-md bg-[#F95C4B] text-white flex items-center justify-center">
                                  <Check className="w-3.5 h-3.5" />
                                </div>
                              ) : (
                                <div className="w-5 h-5 rounded-md border border-[rgba(228,222,210,0.2)] group-hover:border-[#F95C4B] transition" />
                              )}
                            </div>
                          </button>
                        );
                  ...
                  <div className="p-8 text-center space-y-3">
                    <div className="w-12 h-12 rounded-2xl bg-[#161616] border border-[rgba(228,222,210,0.12)] flex items-center justify-center mx-auto text-[#A8A196]">
                      <Search className="w-6 h-6" />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-[#F6F4F1]">
            ...
            {activeTab === "categories" && (
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-12 overflow-hidden divide-y sm:divide-y-0 sm:divide-x divide-[rgba(228,222,210,0.1)]">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-xl text-xs font-medium transition flex items-center justify-between ${
                          isSelectedCat
                            ? "bg-[#181818] text-[#F95C4B] font-bold border-l-2 border-[#F95C4B] shadow-sm"
                            : "text-[#A8A196] hover:bg-[#121212] hover:text-[#F6F4F1]"
                        }`}
                      >
                        ...
                        <div className="flex items-center space-x-1.5 shrink-0 ml-1.5">
                          {selectedCountInGroup > 0 && (
                            <span className="text-[10px] bg-[#F95C4B] text-white px-1.5 py-0.2 rounded-full font-bold">
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-3.5 py-2.5 rounded-xl flex items-center justify-between text-xs transition border ${
                              isSelected
                                ? "bg-[#1C1515] text-[#F95C4B] border-[rgba(249,92,75,0.4)] font-semibold shadow-sm"
                                : "bg-[#121212] hover:bg-[#181818] text-[#F6F4F1] border-[rgba(228,222,210,0.06)] hover:border-[rgba(228,222,210,0.12)]"
                            }`}
                          >
                            ...
                            <div className="shrink-0">
                              {isSelected ? (
                                ...
                                <div className="w-5 h-5 rounded-md border border-[rgba(228,222,210,0.2)]" />
                              )}
                            </div>
                          </button>
                        );
          ...
          {/* Modal Footer */}
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-[rgba(228,222,210,0.12)] bg-[#121212] px-5 sm:px-6 py-3.5">
            <div className="text-xs text-[#A8A196]">
              {selectedIndustryIds.length > 0 ? (
                <span className="font-medium text-[#F6F4F1]">
                  <strong className="text-[#F95C4B]">{selectedIndustryIds.length}</strong> target industry niche(s) selected
                </span>
              ) : (
            ...
            <div className="flex items-center space-x-2 justify-end">
              <button
                type="button"
                onClick={handleClose}
                className="px-4 py-2 text-xs font-semibold text-[#A8A196] hover:text-[#F6F4F1] hover:bg-[#1C1C1C] rounded-xl transition"
              >
                Cancel
              </button>
              ...
              <button
                type="button"
                onClick={handleClose}
                className="px-5 py-2 bg-[#F95C4B] hover:bg-[#E04838] text-white text-xs font-bold rounded-xl shadow-md shadow-[#F95C4B]/20 transition flex items-center space-x-1.5"
              >
    ...
    <div className="relative w-full">
      ...
      >
        <input
          ...
          disabled={disabled}
          readOnly
          className="w-full bg-[#080808] border border-[rgba(228,222,210,0.12)] rounded-xl pl-4 pr-10 py-3 text-sm text-[#F6F4F1] placeholder-[#A8A196]/50 focus:outline-none focus:ring-2 focus:ring-[#F95C4B]/40 focus:border-[#F95C4B] cursor-pointer group-hover:border-[rgba(228,222,210,0.24)] transition"
        />
        ...
        <div className="absolute right-3 top-3.5 flex items-center space-x-1">
          {value ? (
            <button
              ...
              className="text-[#A8A196] hover:text-[#F6F4F1] p-0.5 transition"
              title="Clear input"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <button
              type="button"
              onClick={handleOpenModal}
              className="text-[#A8A196] group-hover:text-[#F6F4F1] transition"
