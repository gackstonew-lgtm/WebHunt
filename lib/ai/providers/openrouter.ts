...
export class OpenRouterProvider implements AIProvider {
  ...
  constructor() {
    ...
    if (key) {
      this.client = new OpenAI({ baseURL: "https://openrouter.ai/api/v1", apiKey: key, maxRetries: 0 });
    }
  }
  ...
  private normalizeError(error: any, modelId: string): AIGatewayError {
    ...
    return new AIGatewayError(error?.message || "OpenRouter Error", category, this.id, modelId, error);
  }
